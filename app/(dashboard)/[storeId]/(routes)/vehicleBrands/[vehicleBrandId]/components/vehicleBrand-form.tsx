...
type VehicleBrandFormValues = z.infer<typeof formSchema>

interface VehicleBrandFormProps {
  initialData: VehicleBrand | null;
};

export const VehicleBrandForm: React.FC<VehicleBrandFormProps> = ({
  initialData
}) => {
  ...
  const onSubmit = async (data: VehicleBrandFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/${params.storeId}/vehicleBrands/${params.vehicleBrandId}`, data);
      } else {
        await axios.post(`/api/${params.storeId}/vehicleBrands`, data);
      }
      router.refresh();
